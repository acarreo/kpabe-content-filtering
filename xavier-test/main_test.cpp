...
void test_serialization_public_key(KPABE_DPVS_PUBLIC_KEY &pub_key);
...
int main() {
    ...
    // basic test
    {
        ...
        std::cout << "master key size = " << master_key.getSizeInBytes() << std::endl;
        std::cout << "decryption key size = " << dec_key.getSizeInBytes() << std::endl << std::endl;


        test_serialization_public_key(pub_key);

        std::cout << std::endl << std::endl;
        test_serialization_master_key(master_key);

        std::cout << std::endl << std::endl;
        test_serialization_decryption_key(dec_key);
...
void test_serialization_master_key(KPABE_DPVS_MASTER_KEY &master_key)
{
    ...
    // serialize master key via ByteString
    {
        auto start = std::chrono::steady_clock::now();
        ByteString raw_data;
        master_key.serialize(raw_data);
        std::cout << "serialize master key via ByteString (no copy) time = "
                    << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start).count() << " us"
                    << std::endl;
        std::cout << "raw key size = " << raw_data.size() << std::endl << std::endl;
        ...
        // deserialize via ByteString
        {
            ...
            ByteString raw_data;
            raw_data.assign(network_bytes, network_bytes + network_bytes_size);
            master_key2.deserialize(raw_data);
            std::cout << "deserialize master key via ByteString (with copy) time = "
                        << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start).count() << " us"
                        << std::endl;
            std::cout << "key size = " << master_key2.getSizeInBytes() << ", equal = " << (master_key == master_key2) << std::endl << std::endl;
        }
        ...
        // deserialize via vector
        {
            ...
            std::vector<unsigned char> raw_data(network_bytes, network_bytes + network_bytes_size);
            master_key2.deserialize(raw_data);
            std::cout << "deserialize master key via vector (with copy) time = "
                        << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start).count() << " us"
                        << std::endl;
            std::cout << "key size = " << master_key2.getSizeInBytes() << ", equal = " << (master_key == master_key2) << std::endl << std::endl;
        }
        ...
        // deserialize via stringstream
        {
            ...
            std::stringstream ss;
            ss.write((char *)network_bytes, network_bytes_size);
            master_key2.deserialize(ss);
            std::cout << "deserialize master key via stringstream (with copy) time = "
                        << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start).count() << " us"
                        << std::endl;
            std::cout << "key size = " << master_key2.getSizeInBytes() << ", equal = " << (master_key == master_key2) << std::endl << std::endl;
        }
        ...
        // deserialize via istream, no copy
        {
            ...
            start = std::chrono::steady_clock::now();
            IMemStream ims(network_bytes, network_bytes_size);
            master_key2.deserialize(ims);
            std::cout << "deserialize master key via custom istream (without copy) time = "
                        << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start).count() << " us"
                        << std::endl;
            std::cout << "key size = " << master_key2.getSizeInBytes() << ", equal = " << (master_key == master_key2) << std::endl << std::endl;
        }
    ...
    // serialize master key via stringsteam (additionnal copy)
    {
        auto start = std::chrono::steady_clock::now();
        std::stringstream ss;
        master_key.serialize(ss);
        std::string raw_data = ss.str();
        std::cout << "serialize master key via stringstream (with copy) time = "
                    << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start).count() << " us"
                    << std::endl;
        std::cout << "raw key size = " << raw_data.size() << std::endl << std::endl;
    ...
    // serialize master key via custom ogsteam (no addtionnal copy)
    {
        auto start = std::chrono::steady_clock::now();
        std::vector<unsigned char> raw_data(master_key.getSizeInBytes(), 0);
        OMemStream oms(raw_data.data(), raw_data.size());
        master_key.serialize(oms);
        std::cout << "serialize master key via custom ostream (no copy) time = "
                    << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start).count() << " us"
                    << std::endl;
        std::cout << "raw key size = " << raw_data.size() << std::endl << std::endl;
    ...
    // serialize master key via oversized custom osteam (no addtionnal copy)
    {
        auto start = std::chrono::steady_clock::now();
        std::vector<unsigned char> raw_data(master_key.getSizeInBytes() + 64, 0);
        OMemStream oms(raw_data.data(), raw_data.size());
        master_key.serialize(oms);
        std::cout << "serialize master key via 'oversized' custom ostream (no copy) time = "
                    << std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - start).count() << " us"
                    << std::endl;
        std::cout << "raw key size = " << raw_data.size() << std::endl << std::endl;
