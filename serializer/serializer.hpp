...
class Serializer {
    ...
    void serializeToBuffer(std::vector<uint8_t> &buffer) const {
			ByteString temp;
			static_cast<const T*>(this)->serialize(temp, BIN_COMPRESSED);
    ...
    void deserializeFromBuffer(const std::vector<uint8_t> &buffer) {
		...
		void serializeToStream(std::ostream& os, CompressionType compress) const {
			ByteString temp;
			static_cast<const T*>(this)->serialize(temp, compress);
